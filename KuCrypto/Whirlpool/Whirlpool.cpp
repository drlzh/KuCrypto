...
namespace KuCrypto
{
    namespace Whirlpool
    {
        namespace Internal
        {
            ...
            VC_INLINE void CorrectEndianness(uint64* out, const uint64* in, uint64 byteCount)

            {
                uint64 i, count = byteCount / sizeof(uint64);
                for (i = 0; i < count; i++)
        ...
        namespace Engine
        {
             ...
             * // Bytes, not bits
             */
            void WHIRLPOOL_add(const unsigned char* input, uint64_t sourceBytes, WHIRLPOOL_CTX* const ctx)
            {
                ...
                else
                {
                    ...
                    if (num != 0)	// process left over data
                    {
                        if (num + len >= 64)
                        {
                            memcpy(data + num, input, (uint64)(64 - num));
                            Internal::HashMultipleBlocks(ctx, dataBuf, 64);
                            input += (64 - num);
                            len -= (64 - num);
                        ...
                        {
                            memcpy(data + num, input, (uint64)len);
                            return;
                        }
                    }
                    ...
                    if (len >= 64)
                    {
                        ...
                        {
                            uint64 leftOver = Internal::HashMultipleBlocks(ctx, (uint64*)input, len);
                            input += (len - leftOver);
                            len = leftOver;
                        }
                        else
            ...
            void WHIRLPOOL_finalize(WHIRLPOOL_CTX* const ctx, unsigned char* result)
            {
                ...
                Internal::CorrectEndianness(dataBuf, dataBuf, 32);
                ...
                Internal::WhirlpoolTransform(stateBuf, dataBuf);
                Internal::CorrectEndianness(stateBuf, stateBuf, 64);
        ...
        )
        {
            ...
            Engine::WHIRLPOOL_init(&ctx);

            Engine::WHIRLPOOL_add(InputData, InputDataSize, &ctx);

            Engine::WHIRLPOOL_finalize(&ctx, OutputDigest);

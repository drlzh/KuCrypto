	...
	namespace Sha
	{
		namespace Sha2_512
		{
            ...
            namespace Engine
            {
                ...
                {
                    sha512_context ctx = { 0 };
                    KuCrypto::RtlSecureZeroMemory(&ctx, sizeof(ctx));

                    int ret;
                    if ((ret = Internal::sha512_init(&ctx))) return ret;
                    if ((ret = Internal::sha512_update(&ctx, message, message_len))) return ret;
                    if ((ret = Internal::sha512_final(&ctx, out))) return ret;
            ...
            {
                Engine::sha512(InputData, InputDataLength, OutputDigest);
            }
		}
	}
}
